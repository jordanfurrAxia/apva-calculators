...
function runCalculation() {
    let incomeOrPurchase = parseInt(
        document.getElementById("incomeOrPurchase").value
    )
    ...
    let withdrawBase = incomeOrPurchase / withdrawPercent
    let interestPercent = glwb == 0 ? 0.06 : 0.07
    let initialPayment =
        withdrawBase / (interestPercent * Math.min(10, deferral) + 1)
    let monthly = incomeOrPurchase / 12
    let percentOfInitial = incomeOrPurchase / initialPayment
    ...
    let outputHTML = `<table>
        ...
        <tr>
            <td>Initial Purchase Payment</td>
            <td>${displayDollars(initialPayment)}</td>
        </tr>
        <tr>
            <td>Monthly</td>
